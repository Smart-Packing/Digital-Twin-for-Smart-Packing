...
Distance are in the order of Kilometers (assuming source is the warehouse [location 0] and the 
destination [location x] is the drop-off point for each package) = location (x-0) = d
...
d = [60, 100, 120] # distance in KM each package needs to travel
p = [10, 20, 30] # length * width * height; dimensions of each package (cm*cm*cm)
V = 50 # Length * Width * Height; Volume of Truck (CM*CM*CM)
n = len(d)
max_distance, packages_into_truck = truckSack(V, p, d, n)

print(max_distance)
print(packages_into_truck)